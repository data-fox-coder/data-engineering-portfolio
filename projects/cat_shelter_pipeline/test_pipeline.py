...
def test_transform_cat_data_success(mock_config):
    ...
    assert (
        "attributes_name" in df.columns
    ), "Nested JSON keys should be flattened with underscores"
    assert (
        "attributes_agegroup" in df.columns
    ), "Column names should be strictly lowercased"
    assert (
        "someextrafield" not in df.columns
    ), "Fields omitted from config should be discarded"
